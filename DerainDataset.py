...
def prepare_data_Rain1400(data_path, patch_size, stride):
    ...
    train_num = 0
    for i in range(400):  # 选取900张图片进行训练
        

        for j in range(14):
            target_file = "%d-%dnorain.jpg" % ((i + 501),j+1)  # 标签文件
            target = cv2.imread(os.path.join(target_path, target_file))
            b, g, r = cv2.split(target)  # 拆分通道
            target = cv2.merge([r, g, b])

            input_file = "%d-%drain.jpg" % (i + 501, j + 1)
